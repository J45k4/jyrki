...
fn todo_item_view(todo_item: &TodoItem) -> Item {
	...
	]).spacing(10).border("1px solid black").padding(5)
}
...
fn tokens_view(project: &Project) -> Item {
	hstack([
		vstack([
			...
			text("Total"),
			text(&format!("{}", project.input_token_count + project.output_token_count)),
		]),
		vstack([
			text("Cost"),
			...
			text("Cost"),
			text(&format_cost(project.input_token_cost + project.output_token_cost)),
		]),
	]).border("1px solid black").padding(5).spacing(15)
}
...
fn project_view(project: &Project) -> Item {
	hstack([
		vstack([
			text_input().placeholder("Instructions").svalue(&project.instructions),
			vstack(project.items.iter().map(|item| {
				match item {
					ConversationItem::ToolUses(tools) => {
						hstack(tools.iter().map(|tool| {
							text(&format!("{:?}", tool)).border("1px solid black").padding(5)
						})).spacing(10)
					},
					ConversationItem::UserMessage(msgs) => {
						vstack([
							text("User"),
							text(&msgs.join(" ")),
						]).spacing(10).padding(5).border("1px solid black")
					},
...
fn nav_item(t: &str) -> Item {
	text(t).padding(10).background_color("#f0f0f0").cursor("pointer").id(SELECT_PROJECT_LINK)
}
...
fn ui(state: &State) -> Item {
	vstack([
		projects_tabs(&state),
		state.active_project.map(|project| project_view(&state.projects[project])).unwrap_or(text("no project selected")),
	]).spacing(10)
}
...
impl App {
	pub fn new() -> App {
		let project = Project {
			...
			todo_items: vec![
				TodoItem { text: "Do this thing".to_string(), done: false },
				TodoItem { text: "Do that thing".to_string(), done: true },
			],
			items: vec![
				ConversationItem::AssistantMessage(vec!["Hello".to_string()]),
				ConversationItem::UserMessage(vec!["Hi".to_string()]),
				ConversationItem::ToolUses(vec![Tool::WriteFile, Tool::ReadFile, Tool::ListFolderContents]),
